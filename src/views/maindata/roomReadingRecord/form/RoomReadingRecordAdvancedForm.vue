<template>
  <a-spin :spinning="loading">
    <page-header-wrapper content="">
      <a-card class="card" title="房间读数主数据" :bordered="false">
        <RoomReadingRecordForm ref="roomReadingRecordForm" :showSubmit="false" />
      </a-card>
...
import { getList, saveOrUpdateComplexData } from '@/api/maindata/RoomReadingRecordApi'
import pick from 'lodash.pick'
import AttachmentTabForm from '@/views/system/attachment/AttachmentTabForm.vue'
import RoomReadingRecordForm from '@/views/maindata/roomReadingRecord/form/RoomReadingRecordForm.vue'

export default {
  name: 'RoomReadingRecordAdvancedForm',
  components: {
    RoomReadingRecordForm,
    AttachmentTabForm,
    FooterToolBar
  },
  data () {
  ...
  methods: {
    // 最终全页面提交
    validate () {
      const headForm = this.$refs.roomReadingRecordForm.form
        ...
        if (!err) {
          ...
          saveOrUpdateComplexData(saveData).then(res => {
            if (res.code !== 200) {
              ...
              this.$refs.roomReadingRecordForm.form.setFieldsValue(pick(res.result, this.$refs.roomReadingRecordForm.fields))
    ...
    loadData () {
      ...
      getList(1, 1, { id: this.id }).then(res => {
        ...
        if (res.code !== 200 || record == null) {
          ...
          this.$refs.roomReadingRecordForm.form.setFieldsValue(pick(record, this.$refs.roomReadingRecordForm.fields))
